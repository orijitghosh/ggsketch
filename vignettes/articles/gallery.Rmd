...
`animate_sketch()` re-renders a plot many times, shifting every roughening seed
per frame, so the whole drawing shimmers like a hand-animated cel. Frames are
stitched into a GIF when **gifski** or **magick** is installed; otherwise the
frame paths are returned. It boils any sketch plot with no change to its code.
