...
`geom_sketch_function()` sketches an analytic curve over the x range — handy for
teaching or overlaying a theoretical density.
...
crisp. Pass `rough_frame = TRUE` and the *frame itself* is roughened too — so the
whole plot, not just the marks, looks drawn by hand.
...
`scale_colour_sketch()` / `scale_fill_sketch()` use a curated paper-and-ink
qualitative palette (`sketch_palette()`) that suits the hand-drawn look:
...
`"scribble"` joins the fill-style family — one continuous winding stroke that
overshoots the boundary, like quickly scribbling to fill a shape:
...
It works anywhere a `fill_style` is accepted — here are the eight styles side by
side:
