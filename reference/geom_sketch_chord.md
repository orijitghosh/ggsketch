...
Draws a hand-drawn chord diagram: nodes are placed on a circle, each
given an arc proportional to its total flow, and every weighted relation
becomes a ribbon whose ends are sub-arcs on the two nodes, joined by
curves through the centre. Like
[`geom_sketch_sf()`](https://orijitghosh.github.io/ggsketch/reference/geom_sketch_sf.md),
this is a constructor that returns a list of ordinary sketch layers
(roughened ribbons, rim segments and handwriting labels), so it composes
with `+`; pair it with
...
- value:
  ...
  Unquoted column name giving the edge weight. Default: every edge
  counts as 1.
...
Supply an edge table as `data` and name the `from`, `to` and `value`
columns (unquoted). Ribbons are coloured by their source node; add
[`scale_fill_sketch()`](https://orijitghosh.github.io/ggsketch/reference/scale_sketch.md)
(or any fill scale) to control the palette.
