...
Draws a hand-drawn mosaic plot of two categorical variables: the width
of each column is the marginal frequency of `x`, and the height of each
tile within a column is the conditional frequency of `y`, so every
tile's *area* is the joint frequency. Like
...
(roughened tiles coloured by `y`, plus column labels), so it composes
with `+` and any fill scale; pair it with
...
- x, y:
  ...
  Unquoted column names: `x` splits the columns, `y` splits each column
  vertically.
...
- fill_by:
  ...
  Which variable colours the tiles: `"y"` (default) or `"x"`.
...
- label:
  ...
  Draw column labels? Default `TRUE`.
