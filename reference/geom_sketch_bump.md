...
Draws a hand-drawn bump chart: each series' rank at every time point,
joined across adjacent times by smooth roughened curves so a crossing
reads as one series overtaking another. Like
...
(connector ribbons, rank points and end labels), so it composes with
`+`; map a colour scale over the series and pair with
...
- x, group, value:
  ...
  Unquoted column names: the (ordinal) time, the series, and the value
  ranked within each time.
...
- direction:
  ...
  `"desc"` (default) ranks the highest `value` as rank 1, drawn at the
  top; `"asc"` flips it.
...
- label:
  ...
  Draw series labels at the first and last time point? Default `TRUE`.
