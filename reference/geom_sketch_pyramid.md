...
first level in sort order grows left, the second right). Usually you
also map `fill` to the same variable as `side`; relabel the mirrored
axis with `scale_x_continuous(labels = abs)`.
...
- mapping:
  ...
  Requires `x` (the count), `y` (the category) and `side` (the two-level
  split).
...
- stat:
  ...
  Statistical transformation. Default `"sketch_pyramid"`.
