...
- medium:
  ...
  (`"pen"`, `"ink"`, `"fountain_pen"`, `"ballpoint"`, `"brush"`,
  `"pencil"`, `"charcoal"`, `"pastel"`, `"chalk"`, `"marker"`,
  `"highlighter"`, `"crayon"`, `"spray"`). `NULL` (default) uses
  `"pen"`, the classic constant-width double stroke; most others render
  through the variable-width
  [`stroke_ribbon()`](https://orijitghosh.github.io/ggsketch/reference/stroke_ribbon.md)
  engine (tapered ink, brushy swells, grainy pencil/charcoal, dusty
  chalk, a translucent highlighter band, ...), and `"spray"` renders as
  a soft airbrush dot cloud
  ([`sketch_spray_grob()`](https://orijitghosh.github.io/ggsketch/reference/sketch_spray_grob.md)).
  `medium` is also a mappable aesthetic: map it with `aes(medium = )`
  (one medium per group) and control the mapping with
...
**along** the line, like a pen pressed harder in places. The line is
then drawn through the variable-width
[`stroke_ribbon()`](https://orijitghosh.github.io/ggsketch/reference/stroke_ribbon.md)
engine even under the default `medium = "pen"`, and combines with any
non-`pen` medium (their width profiles multiply). The mapped values are
rescaled by
[`scale_pressure_continuous()`](https://orijitghosh.github.io/ggsketch/reference/scale_pressure_continuous.md);
wrap in [`base::I()`](https://rdrr.io/r/base/AsIs.html) for raw
multipliers.
