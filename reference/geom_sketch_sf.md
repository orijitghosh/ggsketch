...
A hand-drawn take on
[`ggplot2::geom_sf()`](https://ggplot2.tidyverse.org/reference/ggsf.html).
Roughens the boundaries of sf geometry: `(MULTI)POLYGON` features are
filled with a hole-aware hachure (or any `fill_style`),
`(MULTI)LINESTRING` features become sketch paths, and `(MULTI)POINT`
features become sketch points. One call draws whichever geometry types
are present.
...
- mapping:
  ...
  referring to columns of `data` (e.g. `aes(fill = pop)`). The geometry
  column is handled automatically - do not map it.
...
- show.legend, inherit.aes:
  ...
  Standard layer arguments. `inherit.aes` defaults to `FALSE` (as for
...
Requires the sf package. Unlike
[`geom_sf()`](https://ggplot2.tidyverse.org/reference/ggsf.html), this
does not integrate with
[`coord_sf()`](https://ggplot2.tidyverse.org/reference/ggsf.html); it
plots in **planar coordinates**, so pre-project longitude/latitude data
with
[`sf::st_transform()`](https://r-spatial.github.io/sf/reference/st_transform.html)
for an undistorted map. The `data` argument must be given explicitly (an
`sf` or `sfc` object); aesthetics in `mapping` refer to columns of that
object.
