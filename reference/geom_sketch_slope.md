...
Draws a hand-drawn slopegraph: one roughened line per `group` connecting
its values across an ordered x (commonly two categories, e.g.
before/after), with a sketch point at each vertex. Emphasises rank
changes and rates of change between a small number of timepoints (cf.
Tufte's slopegraph).
...
- mapping:
  ...
  Requires `x` and `y`; map `group` to the series identity (and usually
  `colour`).
