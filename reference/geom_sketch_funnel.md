...
Draws a hand-drawn funnel: one horizontal bar per stage, centred on
zero, whose width is the stage's value (`x`), so the shrinking bars read
as drop-off down the stages (`y`). Translucent trapezoids connect each
stage to the next. Put the first stage at the top by ordering the `y`
factor levels accordingly (or add `scale_y_discrete(limits = rev)`);
hide the mirrored x axis with `theme(axis.text.x = element_blank())` or
relabel it with `scale_x_continuous(labels = abs)`.
...
- mapping:
  ...
  Requires `x` (the stage value) and `y` (the stage).
...
- stat:
  ...
  Statistical transformation. Default `"sketch_funnel"`.
...
- connectors:
  ...
  Draw the trapezoid connectors? Default `TRUE`.
