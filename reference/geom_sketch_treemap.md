...
Draws a hand-drawn treemap: nested rectangles tiling a square, each with
area proportional to a value, so the biggest categories take the most
space. Map the value to `area` and the category to `fill`; map `label`
to write a name in each tile. The rectangles are laid out with a
squarified algorithm
([`treemap_layout()`](https://orijitghosh.github.io/ggsketch/reference/treemap_layout.md))
and drawn with the roughened rect look, so they take any `fill_style`
(including `"watercolor"`). Add
[`ggplot2::coord_equal()`](https://ggplot2.tidyverse.org/reference/coord_fixed.html)
for true proportions. No new dependencies (cf.
...
- mapping:
  ...
  Requires `area`; usually map `fill` and optionally `label`.
