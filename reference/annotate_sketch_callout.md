...
The easiest way to add a boxed note: a single hand-drawn callout (with
an optional leader arrow) that does not inherit the plot's aesthetics. A
thin wrapper around
...
- xend, yend:
  ...
  Optional target the leader points at. `NULL` (default) draws a boxed
...
- ...:
  ...
  [`geom_sketch_callout()`](https://orijitghosh.github.io/ggsketch/reference/geom_sketch_callout.md)
  (e.g. `colour`, `fill`, `roughness`, `corner_radius`, `family`).
