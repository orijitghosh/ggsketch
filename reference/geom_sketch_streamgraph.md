...
Draws a hand-drawn streamgraph (a.k.a. ThemeRiver): a stacked area chart
whose baseline floats so the coloured bands flow around a moving centre,
good for showing how a few categories rise and fall over time. Map `x`
(usually time), `y` (value), and `fill` (category). Values are stacked
and the baseline offset by StatSketchStream; each band is drawn as a
roughened ribbon, so it takes any `fill_style` (including
...
- mapping:
  ...
  Requires `x` and `y`; usually map `fill` (or `group`) to the category.
...
- stat:
  ...
  Statistical transformation. Default `"sketch_stream"`.
...
- offset:
  ...
  Baseline placement: `"silhouette"` (centred, the classic streamgraph,
  default), `"zero"` (a normal stacked area), or `"wiggle"` (minimises
  the overall slope).
