...
Draws a hand-drawn ridgeline plot: a kernel density of `x` for each
category on the discrete `y`, raised to its own baseline so the ridges
overlap and the changing shape of the distribution is easy to compare.
Densities are computed and positioned by StatSketchDensityRidges; each
ridge is filled and outlined with the usual sketch look, and ridges are
drawn back-to-front so nearer ones sit on top (cf.
...
- mapping:
  ...
  Requires `x` and a (usually discrete) `y`.
...
- stat:
  ...
  Statistical transformation. Default `"sketch_density_ridges"`.
...
- scale:
  ...
  Vertical scaling: how many `y` units the tallest ridge spans. Values
  above 1 make ridges overlap. Default 1.8.
...
- rel_min_height:
  ...
  Drop the density tails below this fraction of the global peak,
  trimming long thin feet. Default 0 (keep all).
...
- fill_style:
  ...
  `"scribble"`, `"dots"`, `"dashed"`, `"stipple"`, `"pencil_shade"`,
  `"solid"`, or `"watercolor"`. Default `"hachure"`.
