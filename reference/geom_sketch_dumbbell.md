...
Draws a hand-drawn dumbbell: a roughened connector from `x` to `xend` on
a shared `y`, capped with a sketch point at each end. Ideal for showing
the change or gap between two paired values per category (cf.
`ggalt::geom_dumbbell()`).
