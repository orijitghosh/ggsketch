...
Draws a hand-drawn coxcomb (polar area) chart: each category of `x`
occupies an equal angular wedge and its `value` sets the radius, so the
wedges fan out like a rose. With `area_true = TRUE` the sector *area*
(not radius) encodes value, as in Florence Nightingale's original
mortality roses. An optional `fill` category stacks radially within each
wedge. Like
...
(roughened annular sectors + labels), so it composes with `+`; pair it
with
...
- fill:
  ...
  Optional unquoted column name of a second category to stack radially
  within each wedge (`NULL` = one segment per wedge, coloured by `x`).
