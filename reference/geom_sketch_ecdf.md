...
`geom_sketch_ecdf()` draws the empirical CDF of a continuous variable as
a hand-drawn stairstep - the sketch analogue of
[`ggplot2::stat_ecdf()`](https://ggplot2.tidyverse.org/reference/stat_ecdf.html)
/ `geom_step(stat = "ecdf")`. Needs an `x` aesthetic; each group gets
its own curve.
...
- n:
  ...
  If `NULL` (default), the ECDF jumps at each observed value; otherwise
  the curve is evaluated at `n` evenly spaced points (passed to
  [`ggplot2::stat_ecdf()`](https://ggplot2.tidyverse.org/reference/stat_ecdf.html)).

- pad:

  If `TRUE`, add `-Inf`/`Inf` plateaus at the ends. Default `FALSE` (the
  roughened line spans the data range only, avoiding infinite
  coordinates).
