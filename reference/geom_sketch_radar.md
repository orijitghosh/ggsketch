...
Draws a hand-drawn radar chart: each series is a closed polygon over
evenly spaced angular axes, with a roughened web (concentric rings,
radial spokes, and axis labels) drawn behind. Map `axis` (the variable
that becomes each spoke, usually a factor), `value` (its magnitude), and
`group` (the series); map `colour`/`fill` to the series too. Values are
scaled to a common unit radius (`rmax`), and each polygon takes any
`fill_style` (including `"watercolor"`). No new dependencies (cf.
`ggradar`, `fmsb::radarchart()`).
...
- mapping:
  ...
  Requires `axis` and `value`; usually also map `group` (and
  `colour`/`fill`) to the series.
...
- stat:
  ...
  Statistical transformation. Default `"sketch_radar"`.
...
- rmax:
  ...
  Value mapped to the outer ring (`NULL` = the data maximum).
...
- grid_colour:
  ...
  Colour of the web (rings + spokes). Default `"grey75"`.
...
the chart lives in its own square coordinate space, so pair it with
